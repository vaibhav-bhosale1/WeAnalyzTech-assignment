...
const TechnicalLayout: React.FC<TechnicalLayoutProps> = ({ children }) => {
  const location = useLocation();
  const { user, logout } = useAuth();
  
  const navigation = [
    { name: 'Dashboard', href: '/technical/dashboard', icon: LayoutDashboard },
    { name: 'My Ticket', href: '/technical/my-ticket', icon: Ticket },
    { name: 'Performance', href: '/technical/performance', icon: TrendingUp },
  ];
      ...
      <header className="bg-teal-500 text-white shadow-sm">
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link to="/technical/profile" className="p-2 hover:bg-teal-600 rounded-lg transition-colors">
