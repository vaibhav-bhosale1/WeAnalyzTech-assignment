...
const OperationLayout: React.FC<OperationLayoutProps> = ({ children }) => {
  const location = useLocation();
  const { user, logout } = useAuth();
  
  const navigation = [
    { name: 'Dashboard', href: '/operation/dashboard', icon: LayoutDashboard },
    { name: 'Ticket Approval', href: '/operation/ticket-approval', icon: CheckCircle },
    { name: 'My Ticket', href: '/operation/my-ticket', icon: Ticket },
    { name: 'Performance', href: '/operation/performance', icon: TrendingUp },
  ];
      ...
      <header className="bg-teal-500 text-white shadow-sm">
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link to="/operation/profile" className="p-2 hover:bg-teal-600 rounded-lg transition-colors">
